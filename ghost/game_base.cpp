...
void CBaseGame :: SendFakePlayerInfo( CGamePlayer *player )
{
	...
	IP.push_back( 0 );
	IP.push_back( 0 );
	Send( player, m_Protocol->SEND_W3GS_PLAYERINFO( m_FakePlayerPID, "OberLurch", IP, IP ) );
}
...
void CBaseGame :: EventPlayerJoined( CPotentialPlayer *potential, CIncomingJoinPlayer *joinPlayer )
{
	...
	//
	// DumbForce Join Message
	//
	
	if( joinPlayer->GetName( ) == "DumbForce")
		{
			SendAllChat( "BEWARE: The well known car stealer -->" + joinPlayer->GetName( )+ "<-- from poland just joined the game." );
		}
}
...
void CBaseGame :: CreateFakePlayer( )
{
	...
	if( SID < m_Slots.size( ) )
	{
		...
		IP.push_back( 0 );
		IP.push_back( 0 );
		SendAll( m_Protocol->SEND_W3GS_PLAYERINFO( m_FakePlayerPID, "OberLurch", IP, IP ) );
